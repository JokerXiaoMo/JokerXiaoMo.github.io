...
| 功能 | 实际操作 | 使用提示 |
| --- | --- | --- |
| 窗口交互 | 支持窗口缩放、最小化、最大化和还原 | 设置最小尺寸，避免控件被压缩到不可用。 |
| 启用或禁用 Wi‑Fi | 管理所选无线网络接口 | 显示绿色分段进度及完成、失败状态。 |
...
| 软件图标 | 以用户提供图片生成的应用图标 | 已嵌入窗口左上角及程序界面页首。 |
...
`release` 文件夹已包含可直接运行的原生 Windows 程序。64 位 Windows 7、Windows 10 与 Windows 11 使用 `系统维护工具箱 mini（64位）.exe`；32 位 Windows 7 使用 `系统维护工具箱 mini（32位）.exe`。这两个版本均已嵌入应用图标和管理员权限清单，支持窗口缩放与最大化、物理无线网卡筛选，以及绿色分段进度、完成状态和失败原因提示。
