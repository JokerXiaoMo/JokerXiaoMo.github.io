...
| 策略组 | 用途 | 默认候选顺序 |
|---|---|---|
| `🌸 代理选择` | 常规代理的总开关 | 所有可用地区 → 直连 |
| `🤖 AI 魔法工坊` | ChatGPT、Claude、Google AI 等 | 美 / 新 / 日 / 全部节点 → 直连 |
| `📺 追番放映室` | YouTube、Netflix 等 | 港 / 台 / 日 / 美 / 新 / 全部节点 → 直连 |
| `🔍 Google 魔导书` | Google 与 API | 美 / 日 / 新 / 全部节点 → 直连 |
| `🧰 开发者工坊` | GitHub、GitLab 等 | 美 / 日 / 新 / 全部节点 → 直连 |
| `🌐 异界漫游` | 通用海外网站 | 所有可用地区 → 直连 |
| `🏯 国风直连` | 中国大陆站点与 IP | 直连 → 所有可用地区 |
| `✨ 漏网之鱼` | 未命中规则的最终流量 | 所有可用地区 → 直连 |
| `🛡️ 广告退散` | 广告拦截 | 拒绝 → 直连 |
...
请刷新订阅后确认代理页至少出现 `🌌 全部节点`、`🌸 代理选择`、`🤖 AI 魔法工坊` 与 `✨ 漏网之鱼`。随后检查规则页是否下载了 3 个以 `sakura-` 开头的规则提供者，并访问一个国内站点、一个海外站点与一个 AI/视频服务，按实际解锁能力在策略组中调整地区。
